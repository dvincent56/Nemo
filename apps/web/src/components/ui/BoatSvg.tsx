/**
 * Composant partagé — rendu IMOCA SVG avec couleurs dynamiques.
 * Utilisé sur les cartes marina, le hero détail bateau, le profil, etc.
 */
...
export interface BoatSvgProps {
  /** Couleur principale de la coque. */
  hullColor: string;
  /** Couleur du pont / cabine (default: darken(hullColor, 0.15)). */
  deckColor?: string | undefined;
  /** Numéro de coque affiché sur la voile. */
  hullNumber?: string | undefined;
  /** Nom du bateau affiché sur la coque. */
  name?: string | undefined;
  /** Afficher les textes (numéro, nom). Désactivé par défaut pour les mini vues. */
  showText?: boolean | undefined;
  /** Mode silhouette grisée (slots verrouillés). */
  locked?: boolean | undefined;
  /** className pour le <svg>. */
...
export function BoatSvg({
  ...
  locked = false,
  className,
}: BoatSvgProps): React.ReactElement {
  if (locked) {
    return (
      <svg className={className} viewBox={VB} preserveAspectRatio="xMidYMid meet" aria-hidden>
        <path d={P.mainsail} fill="#1a2840" opacity="0.20" />
        <path d={P.jib}      fill="#1a2840" opacity="0.20" />
        <path d={P.mast}     fill="#1a2840" opacity="0.30" />
        <path d={P.hull}     fill="#1a2840" opacity="0.30" />
        <path d={P.cabin}    fill="#1a2840" opacity="0.25" />
        <path d={P.appendages} fill="#1a2840" opacity="0.18" />
      </svg>
    );
  }

  const cabin = deckColor ?? darken(hullColor, 0.15);
  const sailColor = '#fbf7f0';
  const sailDark  = '#e4ddd0';
  const mastColor = '#1a2840';
  const foilColor = '#c9a227';
  const gradId    = `bg-${hullColor.replace('#', '')}`;

  return (
    <svg className={className} viewBox={VB} preserveAspectRatio="xMidYMid meet" aria-hidden>
      <defs>
        <linearGradient id={gradId} gradientTransform="rotate(90, 0.5, 0.5)">
          <stop offset="0%" stopColor={hullColor} />
          <stop offset="100%" stopColor={darken(hullColor, 0.35)} />
        </linearGradient>
        <linearGradient id={`sail-${gradId}`} gradientTransform="rotate(180, 0.5, 0.5)">
          <stop offset="0%" stopColor={sailColor} />
          <stop offset="100%" stopColor={sailDark} />
        </linearGradient>
      </defs>
      ...
      {/* Voiles */}
      <path d={P.mainsail} fill={`url(#sail-${gradId})`} />
      <path d={P.jib}      fill={`url(#sail-${gradId})`} />

      {/* Mât */}
      <path d={P.mast} fill={mastColor} />

      {/* Coque */}
      <path d={P.hull} fill={`url(#${gradId})`} />

      {/* Cabine */}
      <path d={P.cabin} fill={cabin} />

      {/* Appendices */}
      <path d={P.appendages} fill={foilColor} />

      {/* Textes optionnels */}
      {showText && hullNumber && (
        <>
          <text x="155" y="380" fontFamily="var(--font-display)" fontSize="60"
                fill={hullColor} textAnchor="middle" letterSpacing="0.08em"
      ...
      {showText && name && (
        <text x="370" y="862" fontFamily="var(--font-display)" fontSize="22"
              fill="#f5f0e8" textAnchor="middle" letterSpacing="0.14em">
          {name.toUpperCase()}
        </text>
      )}
    </svg>
